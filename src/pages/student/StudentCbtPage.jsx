...
export default function StudentCbtPage() {
  ...
  const start = useMutation({
    mutationFn: async (exam) => {
      // Always ask the API for the current attempt. If the first start request
      // created an attempt but the response was interrupted, the follow-up
      // GET makes the first click recover cleanly instead of requiring a
      // second click after the list changes to "Resume Exam".
      if (exam.attempt_status === 'in_progress' && exam.attempt_id) {
        return cbtApi.getCbtAttempt(exam.attempt_id);
      }
      const data = await cbtApi.startCbtExam(exam.id);
      if (data?.status === 'in_progress' && (!Array.isArray(data.questions) || !data.questions.length) && data.id) {
        return cbtApi.getCbtAttempt(data.id);
      }
      return data;
    },
    onSuccess: (data) => { setAttempt(data); setCurrent(0); setMessage(''); qc.invalidateQueries({ queryKey: ['student-cbt-available'] }); },
    onError: (e) => setMessage(apiErrorMessage(e, 'Could not open the exam. Please try again.'))
  });
