...
export default function StudentCbtPage() {
  ...
  return (
    <><PageHeader title="CBT Examinations" description="Available computer-based examinations and your completed results." /><div className="p-4 md:p-8 space-y-5">{message && <div className="rounded-lg border border-danger/30 bg-danger/5 text-danger px-4 py-3 text-sm">{message}</div>}<Card><div className="p-4 border-b border-border"><h2 className="font-semibold text-ink">Available Exams</h2></div><div className="divide-y divide-border">{(available.data ?? []).map((exam) => <div key={exam.id} className="p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-3"><div><h3 className="font-semibold text-ink">{exam.title}</h3><p className="text-sm text-muted">{exam.subject?.name} · {exam.question_count} questions · {exam.duration_minutes} minutes</p><p className="text-xs text-muted mt-1">Pass mark: {exam.pass_mark}%</p></div><Button onClick={() => start.mutate(exam.id)} disabled={start.isPending || exam.attempt_status === 'submitted'}>{exam.attempt_status === 'submitted' ? 'Completed' : exam.attempt_status === 'in_progress' ? 'Resume Exam' : 'Start Exam'}</Button></div>)}{!available.isLoading && !(available.data ?? []).length && <div className="p-8 text-center text-sm text-muted">There are no active CBT examinations for your class right now.</div>}</div></Card><Card><div className="p-4 border-b border-border"><h2 className="font-semibold text-ink">My CBT Results</h2></div><div className="divide-y divide-border">{(results.data ?? []).map((r) => <div key={r.id} className="p-4 flex justify-between gap-3"><div><p className="font-medium text-ink">{r.exam?.title}</p><p className="text-sm text-muted">{r.exam?.subject?.name} · {new Date(r.submitted_at).toLocaleString()}</p></div><div className="text-right"><p className="font-semibold text-ink">{r.percentage}%</p><Badge tone={r.passed ? 'success' : 'danger'}>{r.passed ? 'Passed' : 'Not passed'}</Badge></div></div>)}{!results.isLoading && !(results.data ?? []).length && <div className="p-8 text-center text-sm text-muted">No completed CBT results yet.</div>}</div></Card></div></>
  );
}
